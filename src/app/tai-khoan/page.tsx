            ...
            <CardContent className="p-4 pt-0 space-y-1">
              ...
              {mockCategories.map((cat) => (
                <button
                  ...
                  className={cn("w-full text-left px-3 py-2 rounded-[10px] text-sm transition-colors flex items-center justify-between", category === cat.slug ? "bg-[#3B82F6]/10 text-[#3B82F6]" : "text-[#94A3B8] hover:bg-[#1F2937] hover:text-white")}
                >
                  <span>{cat.name}</span>
          ...
          <div className={cn("grid gap-4 lg:gap-6", gridSize === 2 && "grid-cols-1 sm:grid-cols-2", gridSize === 3 && "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3", gridSize === 4 && "grid-cols-2 lg:grid-cols-4")}>
            {filteredProducts.map((product) => (
              <Card key={product.id} hover glow className="group overflow-hidden">
                <Link href={`/tai-khoan/${product.slug}`}>
