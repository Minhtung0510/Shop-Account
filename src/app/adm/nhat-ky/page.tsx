...
const ACTION_CONFIG: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ...
  DELETE: { label: "Xóa", color: "bg-[#EF4444]/10 text-[#EF4444]", icon: Trash2 },
  ...
  ROLE_CHANGE: { label: "Đổi vai trò", color: "bg-[#14B8A6]/10 text-[#14B8A6]", icon: Shield },
};
...
export default function AuditLogsPage() {
  ...
  const handleDeleteOldLogs = async () => {
    if (!confirm("Xóa các log cũ hơn 90 ngày?")) return;
    try {
      const res = await fetch("/api/admin/audit-logs?days=90", { method: "DELETE" });
      if (res.ok) { alert("Đã xóa log cũ"); fetchLogs(); }
    } catch (error) {
        ...
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-[#1E293B] bg-[#020617] px-6 h-16">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" onClick={handleDeleteOldLogs}
              ...
              <Trash2 className="h-4 w-4 mr-2" /> Xóa log cũ
            </Button>
            ...
            <CardContent className="p-0">
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      <tr><td colSpan={5} className="text-center py-8 text-[#64748B]">Không có nhật ký nào</td></tr>
                    ) : (
                        ...
                        return (
                          <tr key={log.id} className="border-b border-[#1E293B] hover:bg-[#1E293B]/50">
                            <td className="px-4 py-3 text-sm text-[#94A3B8]">
                              ...
                              <div className="text-xs text-[#64748B]">{new Date(log.createdAt).toLocaleString("vi-VN")}</div>
                            </td>
                            ...
                            <td className="px-4 py-3 text-sm text-[#94A3B8] capitalize">{log.entityType}</td>
                            <td className="px-4 py-3 text-sm text-[#64748B] font-mono">{log.entityId || "-"}</td>
                          </tr>
                        );
                      })
                    )}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-[#1E293B]">
                  <p className="text-sm text-[#64748B]">Trang {page} / {totalPages}</p>
